...
if (process.argv.length < 8) {
    console.log(`<-- By CaylaBot -->`)
    console.log(`<-- ${core_cpu} concurrent available -->`)
    console.log(`<-- Cookie - *double quote required* -->`)
    console.log(`usage: ${process.argv[1]} <method> <url> <time> <concurrent> <req_per_ip> <proxy_file> <cookie>`)
    process.exit(1)
}
...
try {
    ...
    console.log('proxy or uagent file not found');
    process.exit();
}
